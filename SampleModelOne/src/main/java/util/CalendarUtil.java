...
public class CalendarUtil {
	...
	public static String makeTable(int year, int month, int day, List<CalendarDto> list) {
		...
		String dates = (year + "")+ two(month + "") + two(day+ ""); // 문자열로 바꿔서 넣기
	...
	// 202307051029 -> 2023년 7월 5일 10시 29분 혹은 2023-07-05 10:29
	// 날짜와 시간을 유저가 보기 편하게 만듬
	public static String toDates(String mdate) {
		// 날짜형식
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy년 MM월 dd일 hh시 mm분");
		
		String s= 	mdate.substring(0,4)+ "-" // 0~ 4전까지 yyyy
				+ 	mdate.substring(4,6)+"-" //  MM 
				+   mdate.substring(6,8)+ " "  // dd
				+   mdate.substring(8,10) + ":"  //hh
				+   mdate.substring(10) +  " : 00"; // mm:ss
		Timestamp d =Timestamp.valueOf(s); // 이게 머지?
